...
class Table extends Component {
	...
	handleSortingData = (
		...
		if (typeof this.props.onSortOrderChanged === "function") {
			return this.props.onSortOrderChanged(
				columnSortedAscending,
				columnUnsorted,
				index
			);
	...
	sortAlphaNum = (a, b) => {
		...
		for (
			sortedValue, i;
			sortedValue === 0 && i < actualValue.length && i < nextValue.length;
			i++
		) {
	...
	renderRow = ({ item, index }) => {
		...
		return (
			<TableData
				...
				onCheckboxPress={(val) =>
					this.handleOneOfCheckboxesChecked(val, index, item.rowName)
				}
				...
				dropdown={dropdown}
				handleDropdownOpened={(val) =>
					this.handleOneOfDropdownOpened(val, index, item.rowName)
				}
	...
	renderPaginationComponent = () => {
		...
		return (
			<TablePagination
				{...this.props}
				onChange={(actualPageVisible) =>
					this.setState({ paginationCurrentPage: actualPageVisible })
				}
				...
				onPageChange={(actualPageVisible) =>
					this.handlePageChanged(actualPageVisible)
				}
				onItemRowPressed={(itemsPerPageChoosen) =>
					this.handleItemsPerPageExtend(itemsPerPageChoosen)
				}
	...
	render() {
		...
		const { rowsData, paginationPerPage, headersHeight, contentOffsetX } =
			this.state;
		const tablesWidth =
			(cellWidth + spaceBetweenColumns) * headersData.length + 30;
		const themeObj = getThemeObject(themeName);
		const headerHeight =
			(type === "small" && 40) ||
			((type === "medium" || type === "large" || type === "image") && 55);
						...
						>
							...
							>
								...
								<ColumnHeaderList
									...
									borderTopLeftRadius={
										withPagination && !paginationBelow ? 0 : 6
									}
