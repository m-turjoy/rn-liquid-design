...
// import { TextField, Icon, SmallCalendar } from "../../../src/components";
...
class MultiDatePicker extends Component {
	...
	hideModal = () => {
		...
		const { onEndStartEditing, onEndEndEditing } = this.props;

		if (startText !== undefined && startText.length === 14) {
		...
		if (endText !== undefined && endText.length === 14) {
	...
	updateTextEnd = (text) => {
		...
		this.setState({
			endText:
				text.length === 2 && prevText2 < text.length
					? `${text} / `
					: text.length === 7 && prevText2 < text.length
						? `${text} / `
						: text,
	...
	updateSelectText = (text) => {
		const { startText, prevText } = this.state;
	...
	updateSelectTextEnd = (text) => {
		const { endText, startText, prevText2 } = this.state;
	...
	render() {
		...
		const { isValid, isValidEnd, startText, endText } = this.state;
			...
			<ThemeProvider theme={themeObj}>
				<ContainerWrapper flexDirection="column">
					...
					>
						...
						/>
						<TextFieldWrapper marginLeft={10}>
							<TextField
								error={!isValidEnd}
								errorMessage={errorMessage}
								textInputLabelVisible={withLabel}
								...
								onEndEditing={(event) =>
									event.nativeEvent.text
										? this.updateSelectTextEnd(event.nativeEvent.text)
										: null
								}
								value={endText}
								backgroundColor={colors.sensitiveGreyDefault}
						...
						<IconLabelTouchable
							...
							onPress={() => {
								!disabled ? this.showModal() : null;
							}}
							ref={(ref) => {
								this.labelPosition = ref;
							}}
						>
							<IconWrapper marginLeft={10} top={withLabel ? 30 : 0} height={60}>
								<Icon
									name="calendar"
								...
								<CalendarContainer flexGrow={1}>
									...
									>
										<SmallCalendar
											...
											firstDay={firstDay}
											theme={{
												markColor: colors.vibrantCyanDefault,
												markTextColor: "white",
											}}
										/>
									</CalendarWrapper>
