...
class Notification extends Component {
  ...
  showNotification = (type, notificationLabel) => {
    ...
    const item = {
      type, notificationLabel, key: uuidv1(), id: elements.length
    }
    // eslint-disable-next-line
    const check = elements.filter(el => el.type === type && el.notificationLabel === notificationLabel)
    if (check.length === 0) {
      this.setState({
  ...
  show = (item) => {
    item.type !== 'error' ? this.hideTimeout = setTimeout(() => { this.hide(item) }, this.props.notificationRemoveTimeout) : null
    Animated.parallel([
  ...
  hide = (item) => {
    if (!this.state.elements.filter(e => e.key === item.key).length) return null
            ...
            >
              ...
              >
                <Icon
                  ...
                  color={item.type === 'info' ? notificationInfoIconColor : notificationIconColor}
                />
              </IconWrapper>
              <LabelWrapper >
                <Label
        ...
        >
          ...
          >
            <Icon
              ...
              color={item.type === 'info' ? closingInfoIconColor : closingIconColor}
            />
          </IconWrapper>
        </IconTouchable>
      </Animated.View>
  ...
  render() {
    return (
      ...
      >
        ...
        >
          <Toast>
            {this.state.elements.map((item, index) => this.renderToast(item, index))}
          </Toast>
        </Container>
      </ThemeProvider>
    )
