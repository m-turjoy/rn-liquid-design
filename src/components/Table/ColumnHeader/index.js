...
class ColumnHeader extends Component {
	...
	componentWillReceiveProps(nextProps) {
		if (
			this.props.indexPressed !== nextProps.indexPressed &&
	...
	renderIcon = () => {
		const { arrowIconColor, arrowIconSize, themeObj } = this.props;
		...
		return (
			<AnimatedIconWrapper
				...
				marginLeft={3}
			>
				<Icon name="arrowDown" color={handleIconColor} size={arrowIconSize} />
			</AnimatedIconWrapper>
		);
	};
	...
	renderArrowIcon = () => {
		...
		return dropdown && !checkbox && type !== "image" && index === 0 ? (
			<IconWrapperTouchable
	...
	renderCheckboxIcon = () => {
		...
		return checkbox && !dropdown && type !== "image" && index === 0 ? (
			<IconWrapperTouchable
