...
const renderCheckboxIcon = (
  ...
  return (
    checkbox && data.checked !== undefined && !dropdown && type !== 'image' ?
      <IconWrapperTouchable
        onPress={() => (!data.disabled ? handleCheckboxPressed(onCheckboxPress,
...
const renderDropdownIcon = (
  ...
  return (
    dropdown && !checkbox && data.dropdownInfo !== undefined && type !== 'image' ?
      <IconWrapperTouchable
        onPress={() => (!data.disabled ? handleDropdownPressed(handleDropdownOpened,
          optionOpened) : null)}
      >
        <IconWrapper >
          {optionOpened && !data.disabled ?
            ...
            <Icon
              name='arrowDown'
              size={arrowIconSize}
              color={!data.disabled ? handleIconColor : colors.sensitiveGreyDarker}
              // eslint-disable-next-line
              style={{
                marginLeft: -3
              }}
...
const renderFirstColumn = (
  ...
  >
    <Container
      ...
      justifyContent='center'
    >
      {renderDropdownIcon(dropdown, data, checkbox, arrowIconSize,
          arrowIconColor, type, themeObj, optionOpened, handleDropdownOpened)}
      {renderCheckboxIcon(checkbox, data, dropdown, checkboxIconColor,
          checkboxIconSize, type, themeObj, checkboxChecked, onCheckboxPress)}
      {renderImage(type, data, imageWidth, imageHeight,
          imageResizeMode, imageBorderRadius)}
    </Container>
    <Container
      flex={3}
        // eslint-disable-next-line
    ...
    >
      {renderLabel(data, type, labelFontSize,
          labelFontFamily, labelColor, labelStyle)}
      {renderNameInfo(nameFontSize, nameFontFamily, nameColor,
          nameStyle, data, infoFontSize,
          infoFontFamily, infoColor, infoStyle,
          checkbox, dropdown, type)}
    </Container>
  </FirstColumnCellWrapper>
...
const renderHorizontalData = (
  ...
  return (
    <DataWrapper
      ...
      alignItems='flex-start'
      style={cellStyle}
      minHeight={(type === 'small' && 32) || (type === 'medium' && 48) ||
        ((type === 'large' || type === 'image') && 80)}
    >
      <Container
        paddingVertical={
  ...
  render() {
    ...
    return (
      ...
      >
        <RowTouchable
          // eslint-disable-next-line
          onPress={() => (checkbox ? handleCheckboxPressed(onCheckboxPress,
            checkboxChecked) :
            dropdown ? handleDropdownPressed(handleDropdownOpened,
              optionOpened) : null)}
          underlayColor={
            // eslint-disable-next-line
            (checkbox || dropdown) ? colors.transparent : colors.sensitiveGreyDefault
          }
          activeOpacity={1}
        ...
        >
          <RowWrapper
            backgroundColor={this.state.pressStatus ? colors.sensitiveGreyDefault : wrapperColor}
          >
            <RowList
              ...
              backgroundColor={
                ...
                (checkbox && checkboxChecked) || (dropdown && optionOpened) ?
                  type === 'image' ? null :
                    colors.sensitiveGreyDark : null
              }
              scrollEnabled={false}
              ListHeaderComponent={
                renderFirstColumn(type, spaceBetweenColumns, cellWidth, dropdown, checkbox,
                  cellStyle, data, arrowIconSize, arrowIconColor, themeObj, optionOpened,
                  checkboxIconColor, checkboxIconSize, checkboxChecked, imageWidth,
                  imageHeight, imageResizeMode, imageBorderRadius, labelFontSize,
                  labelFontFamily, labelColor, labelStyle, nameFontSize, nameFontFamily,
                  nameColor, nameStyle, infoFontSize, infoFontFamily,
                  infoColor, infoStyle, onCheckboxPress, handleDropdownOpened)
              }
              renderItem={({ item, index }) => renderHorizontalData(item, index, rowTextFontFamily,
                rowTextFontSize, rowTextColor, rowTextStyle, type, labelFontSize,
                labelFontFamily, labelColor, spaceBetweenColumns, cellWidth,
