...
const CustomLabel = (props) => {
  ...
  const newXSmallMidPosition = () => (x > midBoundaryRight ? -80 : (x < midBoundaryLeft ? 40 : -20))
  const newXLargePosition = () => (x > 250 ? -80 : (x < 120 ? 45 : -10))
  const newX = width < bigScreenBoundary ? newXSmallMidPosition() : newXLargePosition()
  const labelStyle = {
    fontSize: 14,
  ...
  const xSmallMidPathPosition = () => (x > midBoundaryRight ? -133 : (x < midBoundaryLeft ? -8 : -70))

  const xLargePathPosition = () => (x > 250 ? -130 : (x < 120 ? -5 : -60))

  const xSmallMidTextPosition = () => (x > midBoundaryRight ? -112 : (x < midBoundaryLeft ? 10 : -50))
  ...
  return (
    <G>
      ...
      >
        <Path
          x={width < bigScreenBoundary ? xSmallMidPathPosition() : xLargePathPosition()}
        ...
        <Text
          y={-24}
          x={
            width < bigScreenBoundary ? xSmallMidTextPosition() : xLargeTextPosition()
          }
          fontFamily={fonts.Regular}
          fontSize={12}
          textAnchor='start'
