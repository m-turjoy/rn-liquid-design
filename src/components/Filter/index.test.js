...
describe('Multiselect Filter Actions, ', () => {
  ...
  const spyOnClearMultiSelected = jest.spyOn(instance, 'handleClearMultiSelection')
  it('handleMultiOnSelect method twice and adds 2 options to multiselectedState', () => {
    ...
    expect(wrapper.state('multiSelectedList')).toEqual([props.options[0], props.options[2]])
  ...
  it('fire handleMultiOnSelect method and expects to remove selected option from state', () => {
    wrapper.setState({ multiSelectedList: [props.options[0], props.options[1]] })
    expect(wrapper.state('multiSelectedList')).toHaveLength(2)
    instance.handleMultiOnSelect(props.options[0])
  ...
  it('fire handleClearMultiSelection and it removes all selected items from list ', () => {
    wrapper.setState({
      multiSelectedList:
      [props.options[0], props.options[1], props.options[2], props.options[3]]
    })

    expect(wrapper.state('multiSelectedList'))
      .toEqual([props.options[0], props.options[1], props.options[2], props.options[3]])
